  ...
  instantsearch.widgets.hits({
    ...
    templates: {
      item(hit) {
        ...
        // Top row: bill code chip + state pill + current-session pill
        const topParts = [];
        if (billCode) topParts.push(`<span class="card-id">${escapeHtml(billCode)}</span>`);
        if (state) topParts.push(`<span class="card-state">${escapeHtml(state)}</span>`);
        ...
        const stateSiteLink =
          hit["State site"] && hit["State site"].trim() !== ""
            ? `<a href="${escapeHtml(hit["State site"])}" target="_blank" rel="noopener">State site ↗</a>`
            : "";
