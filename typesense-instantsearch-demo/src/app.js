...
const searchConfig = {
  ...
  typoTolerance: 2,
  // IMPORTANT: query_by should be TEXT fields only (no embedding)
...
// ----- Hits-per-page updater (idempotent + preserves current page) -----
...
const search = instantsearch({
  ...
  searchFunction(helper) {
    ...
    // Decide k for vector search
    const vectorK = searchConfig.vectorK || perPage * 100;

    // If there is NO query, explicitly disable hybrid/vector and run normal browse
    if (!query) {
    ...
    // Normalize query + set via helper.setQuery (do NOT set 'q' manually)
    ...
    // Typo tolerance (safe defaults; if your adapter dislikes these, see note below)
    helper.setQueryParameter("typo_tolerance", true);
    helper.setQueryParameter("num_typos", searchConfig.typoTolerance);

    // Optional: short queries lean vector-heavy
  ...
  instantsearch.connectors.connectRefinementList((renderOptions, isFirstRender) => {
    ...
    `;
      ...
      checkbox.addEventListener("change", function () {
        // connectRefinementList refine() toggles
        refine("1");
      });
    }
...
window.addEventListener("load", function () {
  setTimeout(() => updateHitsPerPage(), 500); // shorter delay is fine
});
...
search.addWidgets([
  ...
  instantsearch.widgets.hits({
    ...
    templates: {
      item(hit) {
        ...
        const currentSessionBadge =
          hit["Current legislative session"] === 1
    ...
    // IMPORTANT: remove render() { updateHitsPerPage(); } — it breaks pagination
  }),
